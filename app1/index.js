...
let clientes = [...DATASET_INICIAL];
let map = null;
let markersLayerGroup = null;
let savedZonesLayerGroup = null;
let clientMarkerMap = new Map();
let activeZoneFilter = "all";

// Almacén de geometrías de zonas guardadas por el usuario
let savedZoneGeometries = new Map();
...
/**
 * Renderiza los círculos de zonas que el usuario haya guardado explícitamente.
 */
function renderizarZonasGuardadas() {
 ...
 */
async function leerTablaClientes(table, context) {
  ...
  const nameIdx = headerRow.findIndex((h) => h.includes("client") || h.includes("nombre"));
  const contactIdx = headerRow.findIndex((h) => h.includes("contacto"));
  const cityIdx = headerRow.findIndex((h) => h.includes("ciudad"));
  const latIdx = headerRow.findIndex((h) => h.includes("lat"));
  const lngIdx = headerRow.findIndex((h) => h.includes("long") || h.includes("lng"));
  const zoneIdx = headerRow.findIndex((h) => h.includes("zona") || h.includes("region"));
  const salesIdx = headerRow.findIndex((h) => h.includes("venta") || h.includes("usd") || h.includes("total"));
  const priorityIdx = headerRow.findIndex((h) => h.includes("prioridad"));
 ...
 */
async function sincronizarCambiosHaciaExcel(idsActualizados, nuevaZona) {
  await Excel.run(async (context) => {
    const sheet = context.workbook.worksheets.getActiveWorksheet();
    const tables = sheet.tables;
    tables.load("items/name");
...
function activarModoZona() {
  ...
  const radiusKm = parseFloat(radiusSlider ? radiusSlider.value : "3.5");
 ...
 */
function evaluarClientesEnRadio() {
 ...
 */
async function guardarAsignacionZona() {
 ...
 */
async function descargarRutaAExcel() {
  const rutaOrdenada = obtenerRutaOrdenada();
  ...
  if (typeof Excel !== "undefined" && Excel.run) {
    try {
      await Excel.run(async (context) => {
        ...
        let tableClientes = tables.items.find((t) => t.name === NOMBRE_TABLA_EXCEL) || tables.items[0];
        ...
        let existingRutaTable = tables.items.find((t) => t.name === NOMBRE_TABLA_RUTA);
        ...
        const newRutaTable = sheet.tables.add(targetRange, true);
        newRutaTable.name = NOMBRE_TABLA_RUTA;
        newRutaTable.style = "TableStyleLight10";
