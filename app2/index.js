...
const DATASET_INICIAL = [
  ...
  { paso: 13, hora: "09:52", vehiculoId: "UNIDAD-01", conductor: "Carlos Mendoza", lat: 40.4920, lng: -3.7080, velocidad: 58, combustible: 78, estado: "En Movimiento", ubicacion: "Av. del Manzanares" },
...
// ============================================================================
function formatearHoraExcel(val, textVal) {
 ...
 */
function renderizarPuntosDeParada() {
 ...
 */
async function resaltarFilaEnExcel(rowIndex) {
 ...
 */
async function leerTablaGPS(table, context) {
  ...
  const pasoIdx = headerRow.findIndex((h) => h.includes("paso") || h === "id");
  const horaIdx = headerRow.findIndex((h) => h.includes("hora") || h.includes("time"));
  const vehiculoIdx = headerRow.findIndex((h) => h.includes("vehiculo") || h.includes("unidad"));
  const conductorIdx = headerRow.findIndex((h) => h.includes("conductor") || h.includes("chofer"));
  const latIdx = headerRow.findIndex((h) => h.includes("lat"));
  const lngIdx = headerRow.findIndex((h) => h.includes("long") || h.includes("lng"));
  const speedIdx = headerRow.findIndex((h) => h.includes("velocidad") || h.includes("kmh"));
  const fuelIdx = headerRow.findIndex((h) => h.includes("combustible") || h.includes("fuel") || h.includes("gas"));
  const estadoIdx = headerRow.findIndex((h) => h.includes("estado") || h.includes("status"));
  const ubicacionIdx = headerRow.findIndex((h) => h.includes("ubicacion") || h.includes("direccion") || h.includes("poi"));
