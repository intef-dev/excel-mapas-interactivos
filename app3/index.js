...
let propiedades = [...DATASET_INICIAL];
let map = null;
let markersLayerGroup = null;
let propertyMarkerMap = new Map();

// Estado de Filtros
let currentMaxBudget = 600000;
let currentTypeFilter = "all";

// Estado de Resaltado en Excel
let isSyncingExcelHighlight = false;
let pendingFilterHighlight = null;
 ...
 */
function actualizarEstadoMarcadores() {
  propiedades.forEach(prop => {
    const isWithinBudget = prop.precio <= currentMaxBudget;
    const isMatchingType = currentTypeFilter === "all" || prop.tipo.toLowerCase() === currentTypeFilter.toLowerCase();
    const isActive = isWithinBudget && isMatchingType;
...
// ============================================================================
async function sincronizarResaltadoFiltradoEnExcel(filtradas, idMejorOpcion) {
  ...
  try {
    await Excel.run(async (context) => {
      ...
      let table = tables.items.find((t) => t.name === NOMBRE_TABLA_EXCEL) || tables.items[0];
      if (table) {
        ...
        const headerRow = headers.values[0].map((h) => h.toString().toLowerCase().trim());
        const idIdx = headerRow.findIndex((h) => h === "id" || h.includes("id"));
 ...
 */
async function inicializarDatosDesdeExcel() {
  if (typeof Excel === "undefined" || !Excel.run) return;
  ...
  try {
    await Excel.run(async (context) => {
      ...
      let targetTable = tables.items.find((t) => t.name === NOMBRE_TABLA_EXCEL);
      ...
      if (!targetTable) {
        ...
        const startCell = sheet.getCell(0, 0);
 ...
 */
async function leerTablaInmobiliaria(table, context) {
  ...
  const titleIdx = headerRow.findIndex((h) => h.includes("titulo") || h.includes("propiedad") || h.includes("nombre"));
  const typeIdx = headerRow.findIndex((h) => h.includes("tipo"));
  const neighborhoodIdx = headerRow.findIndex((h) => h.includes("barrio") || h.includes("zona") || h.includes("ciudad"));
  const priceIdx = headerRow.findIndex((h) => h.includes("precio") || h.includes("eur") || h.includes("valor"));
  const sqIdx = headerRow.findIndex((h) => h.includes("superficie") || h.includes("m2") || h.includes("metros"));
  const roomsIdx = headerRow.findIndex((h) => h.includes("hab"));
  const bathsIdx = headerRow.findIndex((h) => h.includes("baño") || h.includes("bano"));
  const sqPriceIdx = headerRow.findIndex((h) => h.includes("precio_m2") || h.includes("m2_precio"));
  const latIdx = headerRow.findIndex((h) => h.includes("lat"));
  const lngIdx = headerRow.findIndex((h) => h.includes("long") || h.includes("lng"));
  const energyIdx = headerRow.findIndex((h) => h.includes("calific") || h.includes("energ"));
  ...
  if (typeof Excel !== "undefined" && Excel.run) {
    try {
      await Excel.run(async (context) => {
        ...
        let targetTable = tables.items.find((t) => t.name === NOMBRE_TABLA_EXCEL) || tables.items[0];
        if (targetTable) {
          await leerTablaInmobiliaria(targetTable, context);
        } else {
          await inicializarDatosDesdeExcel();
  ...
  if (typeof Excel !== "undefined" && Excel.run) {
    try {
      await Excel.run(async (context) => {
        ...
        let tablePrincipal = tables.items.find((t) => t.name === NOMBRE_TABLA_EXCEL) || tables.items[0];
        ...
        let existingSummaryTable = tables.items.find((t) => t.name === NOMBRE_TABLA_RESUMEN);
        if (existingSummaryTable) {
          existingSummaryTable.delete();
          await context.sync();
        }

        // A. Escribir Bloque de KPIs Ejecutivos
        const kpiHeaders = [
        ...
        // B. Escribir y Crear Tabla Oficial 'Propiedades_Filtradas'
        const tableHeaders = [
          "ID",
          "Titulo",
          "Tipo",
        ...
        const tableStartRow = startRow + kpiHeaders.length + 2;
        ...
        const newSummaryTable = sheet.tables.add(targetTableRange, true);
        newSummaryTable.name = NOMBRE_TABLA_RESUMEN;
        newSummaryTable.style = "TableStyleLight10";
